...
const schema = z.object({
  ...
  dataTable: z
    .array(z.any())
    .describe("Data for area chart, such as, [{ x: '2018', y: 99.9 }].")
    .nonempty({ message: "Area chart data cannot be empty." }),
  cell: z
    .object({
      x: z
        .string()
        .nonempty({ message: "面积图维度字段不能为空" })
        .describe("面积图维度字段，必须是数据中包含的字段"),
      y: z
        .string()
        .nonempty({ message: "面积图度量字段不能为空" })
        .describe(
          "面积图度量字段，需要是数值类型的字段，必须是数据中包含的字段"
        ),
      color: z
        .string()
        .optional()
